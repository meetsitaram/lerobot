...
class LeRobotDatasetV2(torch.utils.data.Dataset):
    ...
    *data_dim). The compulsory data keys are:
    ...
    def switch_image_mode(self, image_mode: LeRobotDatasetV2ImageMode):
        if self._image_mode != LeRobotDatasetV2ImageMode.VIDEO:
            raise NotImplementedError
        if image_mode != LeRobotDatasetV2ImageMode.MEMMAP:
            raise NotImplementedError
        if image_mode in self._infer_image_modes():
        ...
        for episode_index in self.get_unique_episode_indices():
            ...
            if not new_memmaps_created:
                ...
                self._save_metadata(data_spec=data_spec)
                new_memmaps_created = True
            for k in self.camera_keys:
                self._data[k][where_episode] = episode_data[k]
        self._image_mode = image_mode
